...
class CommandDispatcher {
    ...
    adjustTimelineEmptyEnd(project) {
    ...
    adjustRemoveEmptyChords(project) {
        for (let nn = 0; nn < project.tracks.length; nn++) {
            let track = project.tracks[nn];
...
const kindDraggableCircle2 = 10;
const kindDraggableSquare2 = 11;
const kindDraggableTriangle2 = 12;
class RightMenuItem {
...
function findNearestFilterByKind(idx, kind) {
    let nearest = 987654;
    ...
    if (globalCommandDispatcher.clipboardData) {
        ...
        for (let ii = 0; ii < globalCommandDispatcher.clipboardData.tracks.length; ii++) {
            ...
            if ((track.performer.state == 0 && noSolo) || track.performer.state == 2) {
                ...
                if (!empty) {
                    let info = { text: track.title, noLocalization: true, itemKind: kindDraggableSquare2 };
                    ...
                    let dragger = new DragMenuItemUtil(tri, info, (xx, yy) => {
                        ...
                        let mm = MMUtil();
                        let barIdx = globalCommandDispatcher.cfg().data.timeline.length - 1;
                        let ww = 0;
        ...
        for (let ii = 0; ii < globalCommandDispatcher.clipboardData.percussions.length; ii++) {
            ...
            if ((percussion.sampler.state == 0 && noSolo) || percussion.sampler.state == 2) {
                ...
                if (!empty) {
                    let info = { text: percussion.title, noLocalization: true, itemKind: kindDraggableTriangle2 };
                    let tri = { x: 0, y: 0, w: 11, h: 1, css: 'pasteDragItem' };
                    let dragger = new DragMenuItemUtil(tri, info, (xx, yy) => {
                        let startX = globalCommandDispatcher.cfg().leftPad + globalCommandDispatcher.cfg().timelineWidth() + globalCommandDispatcher.cfg().padGridFan;
                        let mm = MMUtil();
        ...
        for (let ii = 0; ii < globalCommandDispatcher.clipboardData.filters.length; ii++) {
            ...
            if (filter.state == 0) {
                ...
                if (!empty) {
                    let info = { text: filter.title, noLocalization: true, itemKind: kindDraggableCircle2 };
                    let tri = { x: 0, y: 0, w: 11, h: 1, css: 'pasteDragItem' };
                    let dragger = new DragMenuItemUtil(tri, info, (xx, yy) => {
                        let startX = globalCommandDispatcher.cfg().leftPad + globalCommandDispatcher.cfg().timelineWidth() + globalCommandDispatcher.cfg().padGridFan;
                        let mm = MMUtil();
                        ...
                        let startY = Math.round((yy
                            ...
                            - globalCommandDispatcher.cfg().data.percussions.length * globalCommandDispatcher.cfg().samplerDotHeight) / globalCommandDispatcher.cfg().autoPointHeight);
                        if (startY < 0) {
                            startY = 0;
                        }
    ...
    for (let ii = 0; ii < MZXBX_currentPlugins().length; ii++) {
        ...
        if (purpose == 'Action') {
            ...
            if (purpose == 'Sampler') {
                ...
                let dragger = new DragMenuItemUtil(tri, info, (dx, yy) => {
                    globalCommandDispatcher.exe.commitProjectChanges(['percussions'], () => {
                ...
                if (purpose == 'Performer') {
                    let info = { text: label, noLocalization: true, itemKind: kindDraggableSquare };
                    let square = {
                        x: 0, y: 0,
                        w: 1, h: 1,
                        rx: 1 / 20, ry: 1 / 20,
                    ...
                    if (purpose == 'Filter') {
                        let info = { text: label, noLocalization: true, itemKind: kindDraggableCircle };
                        let circle = { x: 0, y: 0, w: 1, h: 1, rx: 1 / 2, ry: 1 / 2, css: 'rectangleDragItem' };
                        let dragger = new DragMenuItemUtil(circle, info, (dx, yy) => {
                            let xx = dx;
                            if (xx < globalCommandDispatcher.cfg().padGridFan) {
                                xx = globalCommandDispatcher.cfg().padGridFan;
                            }
...
function findPerformerIdxByXYcurZ(dx, dy) {
    let zz = globalCommandDispatcher.renderer.tiler.getCurrentPointPosition().z;
...
class MixerUI {
    ...
    resetEditMark() {
        ...
        if (mark) {
            let mm = MMUtil();
            let barX = 0;
            let cubar = globalCommandDispatcher.cfg().data.timeline[0];
            for (let ii = 0; ii < mark.barIdx; ii++) {
