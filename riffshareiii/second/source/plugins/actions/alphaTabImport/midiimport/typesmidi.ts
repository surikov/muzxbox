function firstDrumKeysArrayPercussionPathsMIDI(midi: number): number {
	let pre = '' + midi;
...
function allPercussionDrumTitlesMIDI(): string[] {
...
class ChordPitchPerformerUtilMIDI {
	checkParameters(parameters: string): { loudness: number, idx: number, mode: 0 | 1 | 2 | 3 | 4 } {
		...
		try {
			...
			if (mode == 1) checked.mode = 1;
			if (mode == 2) checked.mode = 2;
