...
class ControllerChangeEvent implements MidiEvent {
	...
	constructor(fields: { controllerNumber: number; controllerValue: number; channel?: number; delta?: number; }) {
		this.channel = (fields.channel as any) - 1 || 0;
		...
		this.data = Utils.numberToVariableLength(fields.delta as any).concat(this.status | this.channel, this.controllerNumber, this.controllerValue);
	}
}
