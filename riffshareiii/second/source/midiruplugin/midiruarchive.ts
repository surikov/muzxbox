...
class InMIDI {
		...
		if (me.parsedProject) {
			...
			if (me.audioContext) {
				...
				if (result) {
					...
					this.player.startLoopTicks(0, this.duration * this.position / 100, this.duration, (msg: string | null) => {
						if (msg) {
							setTimeout(() => {
								me.startPlay();
							}, 500);
	...
	loadFromFileURL() {
		console.log('loadFromFileURL', this.parsedProject);
	...
	loadFromArray(arrayBuffer: ArrayBuffer, filetitle: string | null) {
		console.log('loadFromArray', arrayBuffer);
		...
		console.log('loadFromArray', this.parsedProject);
	}
