class FilterIcon {
	...
	addFilterSpot( filterTarget: Zvoog_FilterTarget, fanLevelAnchor: TileAnchor, zidx: number) {
		...
		let rec: TileRectangle = { x: xx, y: yy, w: globalCommandDispatcher.cfg().pluginIconSize
		...
		this.addOutputs( filterTarget.outputs, fanLevelAnchor, zidx
	...
	addAutoSpot( filterTarget: Zvoog_FilterTarget, fanLevelAnchor: TileAnchor, zidx: number) {
		...
		fanLevelAnchor.content.push(rec);
		if (zidx < 5) {
			let txt: TileText = { text: '' + filterTarget.kind + ':' + filterTarget.id
		...
		new ControlConnection().addLineFlow( yy + globalCommandDispatcher.cfg().pluginIconSize / 2,  controlLineWidth
		, fanLevelAnchor);
		this.addOutputs( filterTarget.outputs, fanLevelAnchor, zidx
