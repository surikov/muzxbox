...
class MidiParser {
    ...
    convertProject(title: string, comment: string): MZXBX_Project {
		...
		*/
		/*
		let drCount = 0;
		for (let ii = 0; ii < midiSongData.miditracks.length; ii++) {
			let tr: MIDISongTrack = midiSongData.miditracks[ii];
			if (tr.channelNum == 9) {
				drCount = drCount + tr.songchords.length;
			}
		}
		console.log('drums---------------------');
		for (let ii = 0; ii < midiSongData.miditracks.length; ii++) {
			let tr: MIDISongTrack = midiSongData.miditracks[ii];
			if (tr.channelNum == 9) {
				console.log(tr.title, tr.songchords[0].notes[0].points[0].pitch, '' + Math.round(100 * tr.songchords.length / drCount) + '%');
			}
		}
		let insDur = 0;
		for (let ii = 0; ii < midiSongData.miditracks.length; ii++) {
			let tr: MIDISongTrack = midiSongData.miditracks[ii];
			if (tr.channelNum != 9) {
				for (let nn = 0; nn < tr.songchords.length; nn++) {
					let ch = tr.songchords[nn];
					insDur = insDur + ch.notes[0].points[0].durationms;
				}
			}
		}
		console.log('voices----------------');
		for (let ii = 0; ii < midiSongData.miditracks.length; ii++) {
			...
			if (tr.channelNum != 9) {
				...
				for (let nn = 0; nn < tr.songchords.length; nn++) {
					let ch = tr.songchords[nn];
					tdu = tdu + ch.notes[0].points[0].durationms;
				}
				console.log(tr.title, tr.program, '' + Math.round(100 * tdu / insDur) + '%');
			}
        }*/
        console.log('midiSongData', midiSongData);
            ...
            for (let ii = 0; ii < midiTrack.songchords.length; ii++) {
                ...
                if (midiChord.when >= currentTimeMs && midiChord.when < currentTimeMs + measureDurationS * 1000.0) {
                    ...
                    if (trackChord == null) {
                        ...
                        //let kk=(midiChord.when - currentTimeMs) / 1000.0;
                        //let tt=new MZXBX_MetreMath().set({ count: 1, part: 256 }).duration(nextMeasure.tempo);
                        //console.log(trackChord, midiChord.when, tt,midiChord.when/tt);
                    }
                    if (trackChord) {
                        for (let nn = 0; nn < midiChord.notes.length; nn++) {
                            ...
                            let curSlide: MZXBX_Slide = { duration: startDuration.strip(16), delta: 0 };
