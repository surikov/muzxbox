var DX7Loader = /** @class */ (function () {
    ...
    DX7Loader.prototype.scale99 = function (nn) {
        var speed = Math.pow(2, nn * 0.16);
        return speed;
    };
    DX7Loader.prototype.durationDown = function (nn) {
    ...
    DX7Loader.prototype.durationUp = function (nn) {
    ...
    DX7Loader.prototype.levelRatio = function (nn) {
        return nn / 99;
        //let ratio = Math.log(nn + 1) * 14 + nn;
        //return ratio;
    };
    DX7Loader.prototype.slopeDuration = function (r99, from99, to99) {
        //let fromRatio = this.levelRatio(from99);
        //let toRatio = this.levelRatio(to99);
        //let fullRatio = this.levelRatio(99);
        var partDuration = Math.abs(this.levelRatio(from99) - this.levelRatio(to99)) / this.levelRatio(99);
        var fullDuration = this.durationDown(r99);
        if (from99 < to99) {
    ...
    DX7Loader.prototype.convertDX7data = function (dx7preset) {
        var modlev = 2.8;
        var preset = {
        ...
        for (var ii = 0; ii < 6; ii++) {
            var data = dx7preset.operators[ii];
            var attackSlope = this.slopeDuration(data.rates0_99[0], data.levels0_99[3], data.levels0_99[0]);
            var decaySlope = this.slopeDuration(data.rates0_99[1], data.levels0_99[0], data.levels0_99[1]);
            var sustainSlope = this.slopeDuration(data.rates0_99[2], data.levels0_99[1], data.levels0_99[2]);
            var releaseSlope = this.slopeDuration(data.rates0_99[3], data.levels0_99[2], data.levels0_99[3]);
            var operator = {
                constantFrequency: 0,
                frequencyRatio: 0,
            ...
            var freqRatio = 1 / (1 + dx7preset.lfoPitchModDepth0_99 / 99);
    ...
    DX7Loader.prototype.parseSyxFile = function (from, onDone) {
        var _this = this;
        ...
        reader.onload = function () {
            ...
            for (var ii = 0; ii < 32; ii++) {
                var one = _this.parseSysexData(result, ii, from.name);
                var preset = _this.convertDX7data(one);
    ...
    DX7Loader.prototype.pow2x = function (x01, minx, maxx, yratio) {
        if (x01) {
    ...
    DX7Loader.prototype.parseSysexData = function (bankData, patchId, filename) {
