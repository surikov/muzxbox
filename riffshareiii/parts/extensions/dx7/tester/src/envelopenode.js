...
var EnvelopeNode = /** @class */ (function () {
    ...
    EnvelopeNode.prototype.slopeDuration = function (preLevel, nextLevel, sloperate) {
        /*
        let rate = (sloperate > 0) ? sloperate : 1;
        let volDiff = Math.abs(preLevel - nextLevel) / 100;
        let radians = rate * ((Math.PI * 0.5) / 100);
        ...
        */
        var part = Math.abs(preLevel - nextLevel) / 100;
        var steep = 123.45;
    ...
    EnvelopeNode.prototype.mapOutputLevel = function (input, volume) {
        var idx = Math.min(99, Math.max(0, Math.floor(input)));
        var level = OUTPUT_LEVEL_TABLE[idx] / 16; //* 1.27;
        var full = level * volume / 100;
        return full;
    };
    ;
    EnvelopeNode.prototype.setLevelRate = function (level1, rate1, level2, rate2, level3, rate3, level4, rate4, when, duration, volume) {
        var slope1 = this.slopeDuration(level4, level1, rate1);
        var slope2 = this.slopeDuration(level1, level2, rate2);
        var slope3 = this.slopeDuration(level2, level3, rate3);
        var slope4 = this.slopeDuration(level3, level4, rate4);
        //console.log('slopes', slope1, slope2, slope3, slope4);
        var volume1 = this.mapOutputLevel(level1, volume);
        var volume2 = this.mapOutputLevel(level2, volume);
        var volume3 = this.mapOutputLevel(level3, volume);
        var volume4 = this.mapOutputLevel(level4, volume);
