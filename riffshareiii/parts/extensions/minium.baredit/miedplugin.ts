...
class BarTimeEdit extends MZXBX_Plugin_UI {
	...
	mergeBarsIntoSingleBar(startMeasure: number, endMeasure: number) {
	...
	copyPasteEmptyBar(from: number, to: number) {
	...
	refit() {
		let txt = (document.getElementById('refitinput') as any).value;
		let newpart = parseInt(txt.split('/')[1]);
		let newcount = parseInt(txt.split('/')[0]);
		let selfrom = document.getElementById('selfrom');
		let selto = document.getElementById('selto');
		...
		let endMeasure: number = parseInt((selto as any).value) - 1;
		this.mergeBarsIntoSingleBar(startMeasure, endMeasure);
		let leftMetre: Zvoog_MetreMathType = MMUtil().set({ count: newcount, part: newpart });
		let rightMetre: Zvoog_MetreMathType = MMUtil().set(this.currentProject.timeline[startMeasure].metre);
		this.currentProject.timeline[startMeasure].metre = leftMetre.metre();
		//console.log(leftMetre.metre());
		let cumsrnum = startMeasure + 1;
		while (rightMetre.more(leftMetre)) {
			//console.log(cumsrnum, leftMetre.metre(), rightMetre.metre());
