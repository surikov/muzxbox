...
class Track implements Chunk {
	...
	mergeExplicitTickEvents() {
		...
		this.explicitTickEvents.forEach((noteEvent) => {
			...
			noteEvent.buildData().events.forEach((e) => (e as any).buildData(this));
	...
	mergeSingleEvent(event: AbstractEvent): Track {
		...
		for (let i = 0; i < this.events.length; i++) {
			if ((this.events[i].tick as any) > (event.tick as any)) break;
			lastEventIndex = i;
		}
		...
		event.delta = (event.tick as any) - (this.events[lastEventIndex].tick as any);
		...
		for (let i = splicedEventIndex + 1; i < this.events.length; i++) {
			...
			this.events[i].delta = (this.events[i].tick as any) - (this.events[i - 1].tick as any);
		}
		return this;
	}
