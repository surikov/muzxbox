...
class TrackInfo {
	...
	public instrumentArticulations: Map<string, InstrumentArticulationWithPlaybackInfo> = new Map<
		string,
		...
		for (const c of element.childElements()) {
			switch (c.localName) {
				...
				case 'midi-unpitched':
					...
					//console.log('midi-unpitched', c.innerText);
					break;
				case 'volume':
	...
	private parseNote(element: XmlNode, masterBar: MasterBar, track: Track) {
		...
		const ensureBeat = () => {
			...
			if (note !== null) {
				...
				//console.log(note.percussionArticulation,instrumentId,trackInfo.track.name);
			}
