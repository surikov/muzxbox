...
const padSend = {};
Object.keys(KIT).forEach(n => {
	const g = ctx.createGain();
	g.gain.value = SEND_GAIN[5];
	g.connect(bus.verb);
	padSend[n] = g;
});
const variantOf = name => Math.min(4, Math.max(1, (variantSettings[name] | 0) || 1));
...
function renderHit(name, t, vel) {
	...
	activeVoice = voice;
	KIT[name].fns[variantOf(name) - 1](t, vg, ratio);
	activeVoice = null;
...
function renderMutes() {
	document.querySelectorAll('.pad').forEach(p =>
		p.classList.toggle('muted', !!(seq.mutes && seq.mutes[p.dataset.voice])));
}
...
document.querySelectorAll('.pad').forEach(pad => {
	...
	pad.addEventListener('pointerdown', e => {
		e.preventDefault();
		holdConsumed = false;
		if (!document.body.classList.contains('shifted')) trigger(name); // SHIFT layer: tap mutes, long-press deletes, no sound
		flash(pad);
...
const dials = []; // every dial registers here so RND can spin them all
...
document.querySelectorAll('.pad').forEach(pad => {
	...
	const renderV = () => {
		...
		vsel.setAttribute('aria-label',
			vname + ' sound: ' + KIT[vname].names[v - 1].toLowerCase() + ', bank ' + v + ' of 4, activate for next');
	};
	let persistVT = null;
	const setV = v => {
		...
		const badge = pad.querySelector('.badge');
		badge.textContent = KIT[vname].names[v - 1];
		badge.classList.add('show');
		clearTimeout(badge._t);
		badge._t = setTimeout(() => badge.classList.remove('show'), 900);
		if (!applyingAuto) announce(vname + ' ' + KIT[vname].names[v - 1]);
	};
	renderV();
...
let pendingKit = null; // a rolled kit waiting for the next bar line
...
let sched = []; // quantized + sorted copy of events
...
let suppressHit = null; // { pad, t } consumed once within the current pass
let autoIdx = 0; // next automation move in this pass
let autoSched = []; // sorted copy of automation (unquantized, true to gesture)
let applyingAuto = false; // guard: replayed moves must not re-record
let guessTimes = []; // free-tempo take: raw hit times, beats resolved on stop
let guessAuto = []; // ditto for knob moves
...
let wavBits = 16,
	wavRate = 0;
try {
	if (+localStorage.getItem('pads808-wavbits') === 24) wavBits = 24;
} catch (_) {}
try {
	if (+localStorage.getItem('pads808-wavrate') === 44100) wavRate = 44100;
} catch (_) {}
		...
		for (let pass = 0; pass < 2; pass++) {
			...
			for (const it of items) {
				...
				if (it.auto) {
					...
					ekit[it.hit].fns[clampInt(variant[it.hit], 1, 4, 1) - 1](t, vg, ratio);
...
if (wavSheet) {
	wavSheet.addEventListener('click', e => {
...
(function bootScreen() {
	...
	const ring = () => {
		...
		vg.gain.value = 0.85;
		vg.connect(bus.master);
		KIT.cowbell.fns[0](ctx.currentTime + 0.02, vg, 1); // the 808 cowbell, on the roll
	};
	const flyToSeat = () => {
...
if (midiSupported) {
	const onMsg = ev => {
		...
		if (st === 0x90 && vel > 0) {
			...
			if (!pad) return;
			arm();
			trigger(pad, Math.pow(vel / 127, 1.4)); // perceptual-ish curve
...
if (midiSheet) {
	midiSheet.addEventListener('click', e => {
