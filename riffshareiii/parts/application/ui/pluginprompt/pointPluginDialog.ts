class PointPluginDialog {
	...
	sendPointToPlugin() {
		let pluginFrame = document.getElementById("pluginPointFrame") as any;
		if (pluginFrame) {
			let message: MZXBX_MessageToPlugin = { hostData: this.pluginPoint.stateBlob, colors: globalCommandDispatcher.readThemeColors(), screenData: null, langID: labelLocaleDictionary };
			pluginFrame.contentWindow.postMessage(message, '*');
