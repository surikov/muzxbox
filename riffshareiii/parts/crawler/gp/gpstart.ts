...
let inames: ChordPitchPerformerUtilMIDI = new ChordPitchPerformerUtilMIDI();
...
function beat_duration(beat: Beat): Zvoog_MetreMathType {
...
function stringFret2pitch(stringNum: number, fretNum: number, tuning: number[], octave: number, tone: number): number {
...
function findVolumeInstrument(program: number): { idx: number, ratio: number } {
...
function addScoreInsTrack(project: Zvoog_Project, scoreTrack: Track, targetId: string) {
	...
	//let imode = findModeInstrument(scoreTrack.playbackInfo.program);
	let volume = 1;
		...
		for (let ss = 0; ss < scoreTrack.staves.length; ss++) {
			...
			for (let vv = 0; vv < bar.voices.length; vv++) {
				...
				for (let bb = 0; bb < voice.beats.length; bb++) {
					...
					//beat.brushType;
					let beatDuration: Zvoog_Metre = beat_duration(beat);
					...
					for (let nn = 0; nn < beat.notes.length; nn++) {
						...
						if (note.isTieDestination) {
							...
							if (note.slideOrigin) {
								...
								if (note.slideTarget) {
									let targetpitch = stringFret2pitch(note.slideTarget.string, note.slideTarget.fret
...
function convertScore2Project(score: Score) {
...
function arrangeFilters(project: Zvoog_Project) {
...
function arrangeDrums(project: Zvoog_Project) {
	let wwCell = 9;
	let hhCell = 3;
...
function arrangeTracks(project: Zvoog_Project) {
	let wwCell = 9;
	let hhCell = 7;
...
function addRepeats(project: Zvoog_Project, score: Score) {
	let startLoop = -1;
	let altStart = -1;
	let projIdx = 0;
...
function cloneOneMeasure(project: Zvoog_Project, from: number, to: number) {
...
function cloneAndRepeat(project: Zvoog_Project, start: number, altEnd: number, end: number, count: number): number {
	...
	function addHeaderText(text: string, label: string, firstBar: Zvoog_CommentMeasure) {
		if (text) {
			let row = firstBar.points.length;
			firstBar.points.push({ skip: { count: 0, part: 1 }, text: label + ': ' + text, row: row });
		}
	}
	function addBarText(text: string, project: Zvoog_Project, barIdx: number) {
...
function addScoreLyrics(project: Zvoog_Project, score: Score) {
