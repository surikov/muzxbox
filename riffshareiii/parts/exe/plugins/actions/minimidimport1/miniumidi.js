    ...
    simplifyAllBendPaths() {
        ...
        for (var t = 0; t < this.parsedTracks.length; t++) {
            ...
            for (var ch = 0; ch < track.trackChords.length; ch++) {
                var chord = track.trackChords[ch];
                for (var n = 0; n < chord.tracknotes.length; n++) {
                    var note = chord.tracknotes[n];
    ...
    convertProject(parser) {
        ...
        for (var i = 0; i < parser.parsedTracks.length; i++) {
            ...
            for (var ch = 0; ch < miditrack.trackChords.length; ch++) {
                var midichord = miditrack.trackChords[ch];
                var newchord = { when: midichord.startMs, notes: [], channel: midichord.channelidx };
                if (maxWhen < midichord.startMs) {
                    maxWhen = midichord.startMs;
                }
                for (var n = 0; n < midichord.tracknotes.length; n++) {
                    var midinote = midichord.tracknotes[n];
...
function takeNearWhen(when, statArr) {
    ...
    let newWhen = { when: when, notes: [], pitches: [], shape: "" };
...
class Projectr {
    ...
    createProjectDrums(volume, top, drum, timeline, midiTrack, outputId) {
        ...
        let projectDrums = {
            title: midiTrack.title + ' ' + allPercussionDrumTitlesMIDI()[drum],
            measures: [],
            sampler: {
    ...
    createProjectTrack(volume, top, timeline, midiTrack, outputId) {
        ...
        let idata = new ChordPitchPerformerUtilMIDI().dumpParameters(ivolume, iidx, imode);
        let projectTrack = {
            title: midiTrack.title + ' ' + new ChordPitchPerformerUtilMIDI().tonechordinslist()[midiTrack.program],
            measures: [],
            performer: {
    ...
    addStartOrIncrese(probe, starts) {
