declare type CachedVoxEnvelope = {
    audioBufferSourceNode: AudioBufferSourceNode;
    when: number;
    duration: number;
    out: GainNode;
};
declare class PerformerCachedVoxPlugin implements MZXBX_AudioPerformerPlugin {
    out: GainNode;
    wavePath: string;
    waveRatio: number;
    ...
    cloneShifted(from: AudioBuffer): void;
    busy(): string | null;
    ...
    resamplePitchShift(pitchShift: number, numSampsToProcess: number, fftFrameSize: number, osamp: number, sampleRate: number, indata: Float32Array): Float32Array;
    shortTimeFourierTransform(fftBuffer: number[], fftFrameSize: number, sign: number): void;
    doPitchShift(audioBuffer: AudioBuffer, ratio: number): void;
}
declare function createPluginCachedVoxPerf(): MZXBX_AudioPerformerPlugin;
