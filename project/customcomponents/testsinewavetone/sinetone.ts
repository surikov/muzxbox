	...
	reset(context: AudioContext, parameters: string): boolean {
		if (this.player) {
			...
			if (window['PublicDefaultBaseOscillatorPlayer']) {
				...
				(window['PublicDefaultBaseOscillatorPlayer'] as DefaultBaseOscillatorPlayer).setup(context);
			}
			this.player = window['PublicDefaultBaseOscillatorPlayer'] as DefaultBaseOscillatorPlayer;
		}
		if (!(this.out)) {
