...
//export 
function deepMerge<T, U>(target: T, source1: U): T & U;
//export 
function deepMerge<T, U, V>(
	target: T,
	source1: U,
	source2: V
): T & U & V;
//export 
function deepMerge<T, U, V, W>(
	target: T,
	source1: U,
	source2: V,
	source3: W
 ...
 */
//export 
function deepEquals<T>(arrayA: T[], arrayB: T[]): boolean {
	return (
 ...
 */
//export 
function optionsFromArguments<T extends object>(
	defaults: T,
 ...
 */
//export 
function getDefaultsFromInstance<T>(instance: T): BaseToneOptions {
	type ToneClass = {
 ...
 */
//export 
function defaultArg<T>(given: T, fallback: T): T {
	if (isUndef(given)) {
		return fallback;
	} else {
		return given;
 ...
 */
//export 
function omitFromObject<T extends object, O extends string[]>(
	obj: T,
	omit: O
): Omit<T, keyof O> {
	omit.forEach((prop) => {
