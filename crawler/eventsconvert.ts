...
class EventsConverter {
	...
	convertEvents(name: string, filesize: number, reader: MIDIReader): MIDIFileInfo {
		...
		*/
		...
		//this.midiFileInfo.project=project;
	...
	*/
	...
	fillInfoMIDI(project: Zvoog_Project, allNotes: TrackNote[], allTracks: MIDITrackInfo[]) {
		...
		*/
		...
		//this.midiFileInfo.drumPerBar = Math.round(this.midiFileInfo.drumCount / project.timeline.length);
		...
		//avgTempoCategory
		if (avgbpm < 80) this.midiFileInfo.avgTempoCategory = 'very slow'
		else if (avgbpm < 110) this.midiFileInfo.avgTempoCategory = 'slow'
		else if (avgbpm < 140) this.midiFileInfo.avgTempoCategory = 'medium'
		else if (avgbpm < 200) this.midiFileInfo.avgTempoCategory = 'fast'
		else this.midiFileInfo.avgTempoCategory = 'very fast'
		...
		for (let ii = 0; ii < this.midiFileInfo.tracks.length; ii++) {
			...
			if (track.program >= 24 && track.program <= 30 && track.chordDuration > 5 * 1000) {
				//wholeChordDuration = wholeChordDuration + track.chordDuration;
				//chordTracks++;
		...
		//this.midiFileInfo.guitarChordDuration = (wholeChordDuration / chordTracks) / this.midiFileInfo.duration;
		this.midiFileInfo.guitarChordDuration = maxTrackChordDuration / this.midiFileInfo.duration;
		//guitarChordCategory
		if (this.midiFileInfo.guitarChordDuration < 0.1) this.midiFileInfo.guitarChordCategory = 'none'
		else if (this.midiFileInfo.guitarChordDuration < 0.2) this.midiFileInfo.guitarChordCategory = 'few'
		else if (this.midiFileInfo.guitarChordDuration < 0.5) this.midiFileInfo.guitarChordCategory = 'medium'
		else this.midiFileInfo.guitarChordCategory = 'many'
		/*
		let guitarChordCount = 0;
		...
		for (let ii = 0; ii < project.tracks.length; ii++) {
			...
			let iidx = track.performer.data.split('/')[1];
			let path: string = instrs[iidx];
			let program = parseInt(path.substring(0, 3));
			if (program >= 24 && program <= 30) {
				guitarTrackCount++;
				for (let mm = 0; mm < track.measures.length; mm++) {
					let measure = track.measures[mm];
					for (let cc = 0; cc < measure.chords.length; cc++) {
						if (measure.chords[cc].pitches.length > 2) {
							guitarChordCount++;
						}
					}
		...
		this.midiFileInfo.guitarChordCount = (guitarChordCount / guitarTrackCount) / project.timeline.length;
		*/

		//project.tracks.reduce((last, it) => last + 1, 0)
